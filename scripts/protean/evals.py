...
CODES = {
    ...
    'hycc': {
        '4_6': [ '24_8_4_4', '96_20_6_6', '192_36_8_8', '1536_260_12_12'],
...
def run_experiment_3(fam):
    ...
    if fam == 'hysc':
        ...
        folders = [f'../data/protean/{code}/v1', f'../data/protean/{code}/v4.4', f'../data/protean/{code}/v4.4']
        decs = ['chromobius', 'chamberland', 'restriction']
    for (i,f) in enumerate(folders):
        ...
        for mflag in ['', '-mx']:
            cmd = fr'''
                    cd Release
                    mkdir -p {f}/output
                    mpirun -np {nproc} ./pr_base_memory {f} --decoder {dc} --e 25 --pmin 5e-4 --pmax 1e-3 -fix-error {mflag}
                    '''
            print('----------------------------')
            print(cmd)
            os.system(cmd)
