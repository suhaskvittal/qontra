...
void
erase_from_incidence_map(std::unordered_map<vpair_t, size_t>& incidence_map, const vpair_t& e) {
...
bool
remove_widowed_edges(std::unordered_map<vpair_t, size_t>& incidence_map) {
    std::unordered_map<sptr<vertex_t>, size_t> vertex_inc_map;
    ...
    sptr<vertex_t> vcen)
{
    ...
    std::unordered_set<vpair_t> boundary_update;
    for (const auto& [vp, cnt] : x_cc_map) {
        ...
        std::unordered_set<vpair_t> best_update;
        ...
        for (sptr<v_t> s : starting) {
            ...
            std::unordered_set<vpair_t> base_boundary;
            size_t s_inc_cnt = 0;
            ...
            for (sptr<v_t> ss : fgr.get_neighbors(s)) {
                ...
                std::unordered_set<vpair_t> local_boundary_update(base_boundary);
                stim::simd_bits<SIMD_WIDTH> local_corr(base_corr);
                std::vector<face_t> local_applied_faces{ *(s->f_p) };
                std::unordered_set<sptr<v_t>> visited{s};
                ...
                while (bfs.size()) {
                    ...
                    vtils::upd_set_symdiff(local_boundary_update, tmp);
        ...
        vtils::upd_set_symdiff(boundary_update, best_update);
