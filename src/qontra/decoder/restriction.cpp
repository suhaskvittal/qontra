...
RestrictionDecoder::decode_error(stim::simd_bits_range_ref<SIMD_WIDTH> syndrome) {
    ...
    fp_t t;
#endif
    std::unordered_map<sptr<hyperedge_t>, size_t> flag_edge_ctr_map;
    ...
    std::unordered_set<sptr<hyperedge_t>> applied_flag_edges;

    std::set<assign_t> in_cc_assignments;
    for (const component_t& cc : components) {
        int color = cc.color;
        std::unordered_set<vpair_t> edge_set;
        for (const assign_t& m : cc.assignments) {
            ...
            std::unordered_set<vpair_t> tmp = insert_error_chain_into(in_cc_map, m.path, color, m.c1, m.c2);
    ...
    std::unordered_map<vpair_t, size_t> _in_cc_map(in_cc_map),
                              _not_cc_map(not_cc_map);
    std::vector<std::pair<std::unordered_set<vpair_t>, int>> _c_edge_sets(component_edge_sets);
    ...
    std::unordered_set<sptr<hyperedge_t>> visited_flag_edges;
...
RestrictionDecoder::split_assignment(
        ...
        const assign_t& m,
        const std::unordered_map<sptr<hyperedge_t>, size_t>& flag_ctr_map)
{
    assign_t curr;
    curr.c1 = m.c1;
    curr.c2 = m.c2;
...
RestrictionDecoder::lifting(
        ...
        const std::unordered_map<sptr<hyperedge_t>, sptr<hyperedge_t>>& best_rep_map,
        size_t tr) 
{
    fp_t out_log_pr = 0.0;
    ...
    std::unordered_set<sptr<vertex_t>> not_cc_incident, in_cc_incident;
...
RestrictionDecoder::flatten_edge_map(const std::unordered_map<sptr<hyperedge_t>, sptr<hyperedge_t>>& edge_map) {
    std::unordered_map<sptr<hyperedge_t>, sptr<hyperedge_t>> flat_edge_map;
...
make_face(sptr<hyperedge_t> e) {
    ...
    std::unordered_set<int> colors_in_face;
