...
#include "qontra/sim/memory_sim.h"
...
MemorySimulator::run(uint64_t shots) {
    ...
    if (world_rank == 0) {
        bool write_header = vtils::file_exists(config.data_output_file);
        vtils::safe_create_directory(config.data_output_file.c_str());
