...
fp_t
DecodingGraph::compute_renorm_factor(std::unordered_set<uint64_t> flags) {
    fp_t f = 1.0;

    std::unordered_set<uint64_t> remaining_flags = vtils::immd_set_diff(active_flags, flags);
    for (sptr<hyperedge_t> e : nod_edges) {
        if (remaining_flags.empty()) break;
        std::unordered_set<uint64_t> flag_intersect = vtils::immd_set_intersect(e->flags, remaining_flags);
        const size_t nf = flag_intersect.size();
        const fp_t p = e->probability;
        if (nf == e->flags.size()) {
            vtils::upd_set_diff(remaining_flags, flag_intersect);
            f *= p;
        }
    }
    return f;
...
DecodingGraph::dijkstra_(int c1, int c2, sptr<vertex_t> from, sptr<vertex_t> to) {
    ...
    uptr<DijkstraGraph>& dgr = dgr_map[c1_c2];
    std::unordered_map<sptr<vertex_t>, fp_t> dist;
    std::unordered_map<sptr<vertex_t>, sptr<vertex_t>> pred;
#ifdef DECODER_PERF
    timer.clk_start();
#endif
...
DecodingGraph::update_paths(
        ...
        const std::unordered_map<sptr<vertex_t>, fp_t>& dist,
        const std::unordered_map<sptr<vertex_t>, sptr<vertex_t>>& pred)
{
