    ...
    reweigh_for_detectors(reweigh_for_detectors)
{
    ...
    auto ef =
        [&] (fp_t p, std::vector<uint64_t> detectors, std::unordered_set<uint64_t> frames)
        {
            if (p == 0) return;
            ...
            if (detectors.size() == 0) {
                ...
                e->flags = std::unordered_set<uint64_t>(flags.begin(), flags.end());
                e->frames = frames;
                e->probability = p;
        ...
        const std::vector<sptr<hyperedge_t>>& edge_list, size_t flips_per_error) 
{
    ...
    for (EdgeClass& c : classes) {
        ...
        std::unordered_set<uint64_t> all_flags;
