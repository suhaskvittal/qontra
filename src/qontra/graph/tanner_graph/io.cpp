...
update_tanner_graph(TannerGraph& graph, std::string line) {
    ...
    // Create check vertex.
    sptr<vertex_t> check_v;
    if (check_id >= 0) {
        const uint64_t flag = is_x_check ? VERTEX_ID_XPARITY_FLAG : VERTEX_ID_ZPARITY_FLAG;

        check_v = std::make_shared<vertex_t>();
        check_v->qubit_type =
            is_x_check ? vertex_t::type::xparity : vertex_t::type::zparity;
    ...
    while (true) {
        ...
        uint64_t dqid = 
            static_cast<uint64_t>(std::stoi(line.substr(pssi, ssi-pssi))) | VERTEX_ID_DATA_FLAG;
        auto data_v = graph.get_vertex(dqid);
        if (data_v == nullptr) {
            data_v = std::make_shared<vertex_t>();
            data_v->id = dqid;
            data_v->qubit_type = vertex_t::type::data;
