...
RawNetwork::add_flag(sptr<raw_vertex_t> dq1, sptr<raw_vertex_t> dq2, sptr<raw_vertex_t> pq)
{
    ...
    vtils::tlm_put(flag_assignment_map, pq, dq1, fq);
    vtils::tlm_put(flag_assignment_map, pq, dq2, fq);
    return fq;
}
...
RawNetwork::add_proxy(sptr<raw_edge_t> e) {
    ...
    add_edge(e2);
    // Update tracking structures and return.
    vtils::tlm_put(proxy_indirection_map, prxq, q1, q2);
    vtils::tlm_put(proxy_indirection_map, prxq, q2, q1);
    return prxq;
}
