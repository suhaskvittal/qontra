...
to_tanner_graph(uptr<TilingGraph>& gr) {
    ...
    for (sptr<shape_t> s : gr->get_vertices()) {
        auto tx = tgr->make_and_add_vertex( (i++) | VERTEX_ID_XPARITY_FLAG );
        auto tz = tgr->make_and_add_vertex( (i++) | VERTEX_ID_ZPARITY_FLAG );
        ...
        for (uint64_t q : s->qubits) {
            ...
            if (tv == nullptr) {
                tv = tgr->make_and_add_vertex( q | VERTEX_ID_DATA_FLAG );
...
to_tanner_graph(const vtils::Mat2& m) {
    ...
    for (uint64_t i = 0; i < m.n_cols; i++) {
        auto tv = tgr->make_and_add_vertex( i | VERTEX_ID_DATA_FLAG );
        tv->qubit_type = tanner::vertex_t::type::data;
    }
    for (uint64_t i = 0; i < m.n_rows; i++) {
        ...
        for (uint64_t j = 0; j < m.n_cols; j++) {
            if (m(i,j)) {
                auto tv = tgr->get_vertex( j | VERTEX_ID_DATA_FLAG );
                gr->make_and_add_edge(tx,tv);
                gr->make_and_add_edge(tz,tv);
            }
        }
...
to_parity_matrix(uptr<TannerGraph>& gr) {
    auto data_qubits = gr->get_vertices_by_type(tanner::vertex_t::type::data).size();
    auto parity_checks = gr->get_vertices_by_type(tanner::vertex_t::type::xparity).size();
    ...
    for (size_t i = 0; i < np; i++) {
        ...
        for (auto d : gr->get_neighbors(p)) {
            m.set( i, d & VERTEX_ID_NUMBER_MASK, true );
        }
    }
    return m;
}
