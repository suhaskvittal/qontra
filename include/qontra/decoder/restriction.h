...
public:
    ...
    RestrictionDecoder(const DetailedStimCircuit& circuit, bool chamberland=false, bool reweigh_detectors=false)
        :MatchingBase(circuit, 3, reweigh_detectors),
...
protected:
    ...
    void split_assignment(
            std::vector<assign_t>&,
            const assign_t&,
            const std::unordered_map<sptr<gd::hyperedge_t>, size_t>& flag_ctr_map);

    std::vector<component_t> compute_connected_components(const std::vector<assign_t>&);

    std::unordered_set<vpair_t> insert_error_chain_into(
                        std::unordered_map<vpair_t, size_t>& incidence_map,
    ...
    fp_t lifting(
            ...
            const std::unordered_map<sptr<gd::hyperedge_t>, sptr<gd::hyperedge_t>>& best_rep_map,
            size_t tries=0);

    std::unordered_map<sptr<gd::hyperedge_t>, sptr<gd::hyperedge_t>>
        flatten_edge_map(const std::unordered_map<sptr<gd::hyperedge_t>, sptr<gd::hyperedge_t>>&);

    void insert_incident_vertices(
            std::unordered_set<sptr<gd::vertex_t>>&, const std::unordered_set<vpair_t>&, int);
    void insert_incident_vertices(
            std::unordered_set<sptr<gd::vertex_t>>&, const std::unordered_map<vpair_t, size_t>&, int);
    ...
    std::unordered_map<vpair_t, size_t> in_cc_map;
    std::unordered_map<vpair_t, size_t> not_cc_map;
    std::vector<std::pair<std::unordered_set<vpair_t>, int>> component_edge_sets;

    typedef std::tuple<sptr<gd::hyperedge_t>, std::vector<sptr<gd::vertex_t>>, std::unordered_map<vpair_t, size_t>&>
        flag_entry_t;
...
void erase_from_incidence_map(std::unordered_map<vpair_t, size_t>&, const vpair_t&);

void update_correction(
        // To be updated:
        std::unordered_map<vpair_t, size_t>& inc_map,
        stim::simd_bits_range_ref<SIMD_WIDTH> corr,
        fp_t& corr_log_pr,
        // Updated by:
        stim::simd_bits_range_ref<SIMD_WIDTH> local_corr,
        const std::unordered_set<vpair_t>& local_boundary,
        fp_t local_log_pr);

bool remove_widowed_edges(std::unordered_map<vpair_t, size_t>&);

std::unordered_set<vpair_t>
find_face_subset_given_cc_map(
        const std::unordered_map<vpair_t, size_t>& x_cc_map,
