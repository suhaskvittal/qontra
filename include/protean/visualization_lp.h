...
typedef CPXLPManager<lp_label_t> LP;

lp_label_t  get_x(sptr<net::phys_vertex_t>);
lp_label_t  get_y(sptr<net::phys_vertex_t>);

lp_expr_t   get_distance_metric(lp_var_t, lp_var_t, lp_var_t, lp_var_t);
...
lp_expr_t   lp_add_crossing_edges_objective(LP&, PhysicalNetwork&, placement_config_t);
lp_expr_t   lp_add_edge_distance_objective(LP&, PhysicalNetwork&);
