...
typedef std::tuple<
            ...
            stim::simd_bits<SIMD_WIDTH>> error_record_t;
...
std::vector<enumerator::error_record_t> enumerate_errors(const schedule_t& prog);

void write_recorded_errors_to(
        std::ostream&,
        const std::vector<enumerator::error_record_t>&);
